...
import { CalendarIcon, User, CheckCircle2, FileUp, X, Banknote, Landmark } from "lucide-react";
...
export default function EditCustomerPayment({ payment, onClose, onSuccess }) {
  ...
  const remainingFor = (sale) => sale.sellPrice - sale.paidAmount + (oldAllocationMap.get(sale.id) || 0);

  const selectedAllocations = useMemo(
    () => Object.entries(selections).filter(([, v]) => v.checked).map(([saleId, v]) => ({ saleId, amount: parseFloat(v.amount) || 0 })),
    [selections],
  );
  ...
  const { label: balLabel, cls: balCls } = balMeta(currentBalance, "customer", null);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/sales/customerSales?customerId=${entity.id}&paymentStatus=DUE,PARTIAL`, { headers: authHeaders() });
        const json = await res.json();
        let list = (json.data || []).map(normalizeSale);

        const missing = [...oldAllocationMap.keys()].filter((id) => !list.some((s) => s.id === id));
        if (missing.length) {
          const fetched = await Promise.all(
            missing.map((id) => fetch(`${API_BASE}/api/sales/saleId/${id}`, { headers: authHeaders() }).then((r) => r.json())),
          );
        ...
        list.forEach((s) => {
          ...
          sel[s.id] =
            oldAmt !== undefined
              ...
              : { checked: false, amount: (s.sellPrice - s.paidAmount).toFixed(2) };
        });
        setSelections(sel);
      } finally {
  ...
  const toggleSale = (sale) =>
    setSelections((prev) => {
      ...
      return { ...prev, [sale.id]: { checked: !cur.checked, amount: !cur.checked ? remainingFor(sale).toFixed(2) : cur.amount } };
    });
  const updateAmount = (saleId, value) => setSelections((prev) => ({ ...prev, [saleId]: { ...prev[saleId], amount: value } }));
  ...
  const save = () =>
    new Promise(async (resolve, reject) => {
      try {
        setPayError("");
        const attachmentUrl = file ? await uploadAttachment(file, setUploading) : (payment.attachmentUrl ?? null);
  ...
  if (success)
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <h3 className="font-bold text-slate-800 text-xl mb-1">Payment Updated!</h3>
        <p className="text-slate-500 text-sm mb-1">
          {fmt(totalAmount)} recorded for <span className="font-medium">{name}</span>
        </p>
        <p className="text-xs text-slate-400 mb-6">
          New balance:{" "}
          <span className={cn("font-semibold", newBalance > 0 ? "text-red-600" : newBalance < 0 ? "text-blue-600" : "text-green-600")}>
            {newBalance < 0 ? "−" : ""}
            {fmt(newBalance)}
          </span>
        </p>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              ...
              <span className={cn("font-medium", currentBalance > 0 ? "text-red-600" : currentBalance < 0 ? "text-blue-600" : "text-green-600")}>
                {currentBalance < 0 ? "−" : ""}
                {fmt(currentBalance)}
              </span>
            </p>
        ...
        <span className={cn("text-xs font-semibold px-2.5 py-1 rounded-full border", balCls)}>{balLabel}</span>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="p-5">
          <h3 className="font-semibold text-slate-700 text-sm mb-3">Invoices</h3>
          {salesLoading ? (
            ...
            <div className="space-y-2.5 max-h-80 overflow-y-auto pr-1">
              {sales.map((sale) => {
                const sel = selections[sale.id] ?? { checked: false, amount: "0" };
                const max = remainingFor(sale);
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      sel.checked ? "border-violet-200 bg-violet-50/50" : "border-slate-200 bg-white",
                      invalid && "border-red-300 bg-red-50/50",
                    )}
                  >
                    <div className="flex items-start gap-3">
                      <Checkbox checked={sel.checked} onCheckedChange={() => toggleSale(sale)} className="mt-1" />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-semibold text-slate-800">Invoice {sale.invoiceNo}</p>
                        ...
                        {sel.checked && (
                          <div className="relative mt-2">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs">SAR</span>
                            <Input
                              type="number"
                              min="0"
                              step="0.01"
                              value={sel.amount}
                              onChange={(e) => updateAmount(sale.id, e.target.value)}
                              onBlur={() => clampAmount(sale)}
                              className="pl-10 h-9 text-sm"
                            />
                          </div>
                        )}
                        {invalid && <p className="text-[11px] text-red-500 mt-1">Must be &gt; 0 and ≤ {fmt(max)}.</p>}
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div
              className={cn(
                "rounded-xl border p-3.5",
                newBalance > 0 ? "bg-amber-50 border-amber-100" : newBalance < 0 ? "bg-blue-50 border-blue-100" : "bg-green-50 border-green-100",
              )}
            >
              <p className={cn("text-[11px] mb-1", newBalance > 0 ? "text-amber-500" : newBalance < 0 ? "text-blue-500" : "text-green-500")}>
                Balance After
              </p>
              <p className={cn("font-bold", newBalance > 0 ? "text-amber-700" : newBalance < 0 ? "text-blue-700" : "text-green-700")}>
                {newBalance < 0 ? "−" : ""}
                {fmt(newBalance)}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={cn(
                  ...
                  paymentMethod === "CASH" ? cn("border-transparent text-white", accentBtn) : "border-slate-200 text-slate-600 hover:bg-slate-50",
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "flex items-center justify-center gap-2 h-10 rounded-xl border text-sm font-medium",
                  paymentMethod === "BANK_TRANSFER" ? cn("border-transparent text-white", accentBtn) : "border-slate-200 text-slate-600 hover:bg-slate-50",
                )}
              >
          ...
          <div className="space-y-1.5">
            ...
            <Popover open={calOpen} onOpenChange={setCalOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-start font-normal h-10 text-sm text-slate-600">
          ...
          <div className="space-y-1.5">
            <Label className="text-sm font-medium text-slate-700">
              ...
              {payment.attachmentUrl && !file && (
                <span className="ml-2 text-xs text-slate-400 font-normal">(existing file kept — upload to replace)</span>
              )}
            </Label>
            <label className="flex items-center gap-3 cursor-pointer rounded-xl border-2 border-dashed border-slate-200 p-3.5 hover:border-slate-300 hover:bg-slate-50">
              <div
                className={cn(
                  ...
                  file ? "bg-green-100" : payment.attachmentUrl ? "bg-blue-50" : "bg-slate-100",
                )}
              >
                {file ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <FileUp className="w-4 h-4 text-slate-400" />}
              </div>
              <div className="flex-1 min-w-0">
                {file ? (
                  <p className="text-sm font-medium text-green-700 truncate">{file.name}</p>
                ) : payment.attachmentUrl ? (
                  <p className="text-sm text-blue-600 font-medium truncate">{payment.attachmentUrl.split("/").pop()}</p>
                ) : (
                  <p className="text-sm text-slate-600 font-medium">Click to upload</p>
                )}
              </div>
              {file && (
                <button type="button" onClick={(e) => { e.preventDefault(); setFile(null); }} className="text-slate-300 hover:text-red-400">
                  <X className="w-4 h-4" />
                </button>
              )}
              <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => e.target.files[0] && setFile(e.target.files[0])} className="hidden" />
            </label>
          </div>
          ...
          {payError && <p className="text-xs text-red-500 bg-red-50 border border-red-100 rounded-xl px-3.5 py-2.5">{payError}</p>}

          <SlideButton
            handlePayment={save}
            disabled={totalAmount <= 0 || hasInvalid || uploading || (paymentMethod === "BANK_TRANSFER" && !bankId)}
            price={totalAmount}
            mode="customer"
          />
        </div>
